...
export async function hubspotOAuthRoutes(app: FastifyInstance): Promise<void> {
  ...
  // Accepts either:
  // 1. connect_token query param (generated by hubspot_connect MCP tool)
  // 2. Bearer token in Authorization header
  app.get('/auth/hubspot', async (request: FastifyRequest<{
    Querystring: { connect_token?: string }
  }>, reply: FastifyReply) => {
    let email: string;
    let bearerToken: string | undefined;

    // Try connect_token first (from hubspot_connect MCP tool)
    const connectToken = request.query.connect_token;
    if (connectToken) {
      // Look up the Bearer token from the connect token
      const result = await dynamodb.send(new GetItemCommand({
        TableName: SESSIONS_TABLE,
        Key: { sessionId: { S: `HUBSPOT_CONNECT#${connectToken}` } }
      }));

      if (!result.Item || !result.Item.bearerToken?.S) {
        return reply.code(401).send({
          error: 'invalid_token',
          message: 'Invalid or expired connection link. Please generate a new one using the hubspot_connect tool.'
        });
      }

      // Delete the connect token (single-use)
      await dynamodb.send(new DeleteItemCommand({
        TableName: SESSIONS_TABLE,
        Key: { sessionId: { S: `HUBSPOT_CONNECT#${connectToken}` } }
      }));
      ...
      if (!authHeader?.startsWith('Bearer ')) {
        return reply.code(401).send({
          error: 'unauthorized',
          message: 'Use the hubspot_connect MCP tool to get a connection URL'
        });
      }
